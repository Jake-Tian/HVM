...
# Re-run reasoning for all 176 questions in the 13-video evaluation set.
# Existing graph memory, episodic memory, and frames are reused. Memorization is
# never invoked by this script.
#
# Usage:
#   bash scripts/reasoning/run_accuracy_test_all_questions.sh
#   DRY_RUN=1 bash scripts/reasoning/run_accuracy_test_all_questions.sh
#   JOBS=2 OUT_DIR=data/ablation/reasoning_accuracy_full_test_gpt5mini_v3 \
#     bash scripts/reasoning/run_accuracy_test_all_questions.sh
...
OUT_DIR="${OUT_DIR:-data/ablation/reasoning_accuracy_full_test_gpt5mini_v2}"
LOG_TAG="${LOG_TAG:-accuracy_full_test_gpt5mini_v2}"
RUNNER_LOG_DIR="${RUNNER_LOG_DIR:-data/logs/accuracy_full_test_gpt5mini_v2}"
...
if [[ ! -f data/robot.json ]]; then
  echo "Missing required input: data/robot.json" >&2
  missing=1
fi
if (( missing )); then
  exit 1
...
if [[ "$selected_total" != "176" ]]; then
  echo "Expected 176 questions across the 13 videos, found ${selected_total}." >&2
  exit 1
fi
...
for path in sorted(out_dir.glob("*.json")):
    ...
    for record in records.values():
        ...
        video_tokens += int(
            (reasoning.get("token_summaries") or {}).get("total", 0) or 0
        )
    total += video_total
...
if total != 176:
    raise SystemExit(f"Expected 176 completed results, found {total}.")
PY
